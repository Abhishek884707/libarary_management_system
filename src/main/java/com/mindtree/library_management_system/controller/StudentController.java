...
import com.mindtree.library_management_system.dto.StudentDto;
import com.mindtree.library_management_system.model.Student;
import com.mindtree.library_management_system.service.StudentService;
import com.mindtree.library_management_system.utilities.StudentUtility;
...
public class StudentController {
    ...
    @GetMapping("/student/search/{search}")
    public List<StudentDto> searchStudentByName(@PathVariable("search") String search){
        List<Student> students = studentService.getStudentsByName(search);
        List<StudentDto> studentDtoList = students.stream().map(student -> StudentUtility.fromStudentToStudentDto(student)).collect(Collectors.toList());
        return studentDtoList;
    }
